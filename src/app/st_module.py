...
from src.core.workflow import Drafter, Persona
...
def check_login(db, user_nickname, user_password):
    ...
    if all_nicknames is None or user_nickname not in all_nicknames:
        return False

    db_password_hash = db.loop.run_until_complete(
        db.fetch_password_by_nickname(user_nickname)
    )
...
def wrap_user(
    user_name: str, user_lastname: str, user_nickname: str, user_password: str
):
    user_id = uuid4()
    user_password_hash = hashlib.sha256(user_password.encode("utf-8")).hexdigest()
    user = User(
