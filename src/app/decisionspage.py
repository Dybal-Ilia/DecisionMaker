...
def render_decisionpage():
    ...
    with st.container(border=True):
        ...
        if st.button("Generate Draft"):
            ...
            st.success("We've prepared a draft for better observability. Make sure to fill up follow-up questions and click 'Make a Decision' if everythin is correct")
            st.divider()
            st.markdown(f"### {draft.general_intent} Draft")

            st.markdown("#### Options:")
                
            options_text = " &nbsp;&nbsp;|&nbsp;&nbsp; ".join([f"*{opt}*" for opt in draft.options])
