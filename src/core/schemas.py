...
class DecomposerResponse(BaseModel):
    instructions: list[str] = Field(
        description="Numbered list of atomic requirements extracted from the query or Implicit assumptions about user intent"
    )


class ReflectorResponse(BaseModel):
    score: int = Field(
        default=0, description="Numeric integer response score from 1 to 10"
    )
    corrections: list[str] = Field(
        default_factory=list,
        description="A list of corrections that should be applied to agents response",
    )
...
class ArxiveArticle(BaseModel):
    ...
    summary: str = Field(
        ..., description="A short summary of the article provided by arxiv"
    )
...
class ArxivSearchResult(BaseModel):
    ...
    articles: list[ArxiveArticle] = Field(
...
class DecisionDraft(BaseModel):
    general_intent: str = Field(
        ...,
        description="User's general intent. It's short and accurate."
        "For example if user queries about vacation and going abroad it should be 'Travelling', etc.",
    )
    options: list[str] = Field(
        ...,
        description="A list of options the user provided. For example if the query is like"
        "'I'm planning to buy a car and cant decide between audi and bmw' then the options are [audi, bmw]",
    )
    preferences: list[str] = Field(
        ...,
        description="A list of preferences provided by the user. It can be anything that the user mentioned. For example if the query is like:"
